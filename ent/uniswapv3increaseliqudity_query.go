...
func (uvlq *UniswapV3IncreaseLiqudityQuery) sqlAll(ctx context.Context) ([]*UniswapV3IncreaseLiqudity, error) {
	...
	if query := uvlq.withEvent; query != nil {
		...
		query.Where(predicate.Event(func(s *sql.Selector) {
			s.Where(sql.InValues(uniswapv3increaseliqudity.EventColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.uniswap_v3increase_liqudity_event
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "uniswap_v3increase_liqudity_event" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "uniswap_v3increase_liqudity_event" returned %v for node %v`, *fk, n.ID)
			}
